...
for k in range(16):
    ...
    b_width = int(75/PixelSize*bmp.shape[1])
    b_height = int(75/PixelSize*bmp.shape[0])
...
def matching(form, perf, bound, y_avg, x_avg):
    ...
    # if the form dimensions deviate too much from the ideal, return -1
    if abs(((max(form[1])-min(form[1]))/(max(perf[1])-min(perf[1])))-1)>1:
        return -1
    if abs(((max(form[0])-min(form[0]))/(max(perf[0])-min(perf[0])))-1)>1:
        return -1
